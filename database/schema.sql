...
CREATE TABLE IF NOT EXISTS `servers` (
  ...
  `tcp_connections` INT NULL COMMENT 'TCP 连接数（inuse）',
  `udp_connections` INT NULL COMMENT 'UDP 连接数（inuse）',
  `system_info` JSON NULL COMMENT '静态系统信息（内核/架构/CPU/DNS/拥塞算法/公网IP/位置/运营商）',
  `static_info_updated_at` DATETIME NULL COMMENT '静态信息最后采集时间',
...
CREATE TABLE IF NOT EXISTS `alert_rules` (
  ...
  `action_ids` JSON NULL COMMENT '命令模板 ID 数组，命中时在目标服务器执行',
  `enabled` TINYINT NOT NULL DEFAULT 1,
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX `idx_type` (`type`),
...
CREATE TABLE IF NOT EXISTS `alert_logs` (
  ...
  `action_results` TEXT NULL COMMENT '动作执行结果 JSON',
  `diagnosis` MEDIUMTEXT NULL COMMENT 'LLM 诊断结果（可选）',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
