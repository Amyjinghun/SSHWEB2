...
function normalizeExportRow(row, includeCredentials = false) {
  ...
  return {
    ...
    private_key_passphrase_encrypted: row.private_key_passphrase_encrypted || '',
    ...
    remark: row.remark || '',
    has_password: !!(password || row.password_encrypted),
    has_private_key: !!(privateKey || row.private_key_encrypted)
  };
}

function buildExportCsv(rows) {
  const headers = [
    '服务器名称','主机地址','端口','用户名','认证方式','密码','私钥','私钥密码','密码(加密)','私钥(加密)','私钥密码(加密)','分组','标签','到期日期','系统版本','状态','CPU','内存','磁盘','最后连接时间','备注'
  ];
  const lines = [headers.join(',')];
  for (const r of rows) {
    lines.push([
      r.name, r.host, r.port, r.username, r.auth_type, r.password, r.private_key, r.private_key_passphrase,
      r.password_encrypted, r.private_key_encrypted, r.private_key_passphrase_encrypted,
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i += 1) {
      try {
        ...
        const plainPassword = pickImportValue(row, ['password', '密码'], '');
        const plainKey = pickImportValue(row, ['private_key', '私钥'], '');
        const plainKeyPass = pickImportValue(row, ['private_key_passphrase', '私钥密码'], '');
        const encPassword = pickImportValue(row, ['password_encrypted', '密码(加密)', '密码_encrypted'], '');
        const encKey = pickImportValue(row, ['private_key_encrypted', '私钥(加密)', '私钥_encrypted'], '');
        const encKeyPass = pickImportValue(row, ['private_key_passphrase_encrypted', '私钥密码(加密)', '私钥密码_encrypted'], '');

        let storedPassword = null, storedKey = null, storedKeyPass = null;
        if (encPassword) storedPassword = encPassword;
        else if (plainPassword) storedPassword = encrypt(plainPassword);
        if (encKey) storedKey = encKey;
        else if (plainKey) storedKey = encrypt(plainKey);
        if (encKeyPass) storedKeyPass = encKeyPass;
        else if (plainKeyPass) storedKeyPass = encrypt(plainKeyPass);
